...
DEFAULT_CLIP_HEIGHT = 1876
DEFAULT_CLIP_WIDTH = 2896
...
class MovieLayout(object):
    ...
    @property
    def scale(self):
        # Return scale of new video based on 2896x1876 video = scale:1
        return (self.video_height * self.video_width) / (2896 * 1876)
...
class FullScreen(MovieLayout):
    ...
    @property
    def video_width(self):
        print('self.cameras("front").width', self.cameras("front").width)
        return int(
            max(
    ...
    def video_height(self):
        perspective_adjustement = 3 / 2 if self.perspective else 1
        print('self.cameras("front").height', self.cameras("front").height)
        return int(
    ...
    def _front_height(self):
        ...
        print('self.cameras("front").height 1', self.cameras("front").__dict__)
        return int(self.cameras("front")._height)
...
class WideScreen(FullScreen):
    ...
    # Only front_width has to be adjusted as by default width would be
    # left+rear+right instead of normal scale.
...
class Diamond(Cross):
    ...
    def _font_size(self):
        ...
        # Thus overriding font size to get video height without font size to
        # figure our scaling.
...
def get_movie_files(source_folder, video_settings):
    ...
    folder_list = set()
    # Determine all the folders to scan for files. Using a SET ensuring
    # uniqueness for the folders.
    ...
    for event_folder in sorted(folder_list):
        ...
        if os.path.isdir(event_folder):
            ...
            for clip_filename in glob(os.path.join(event_folder, "*.mp4")):
                ...
                # Get meta data for each camera for this timestamp to determine creation
                # time and duration.
                    ...
                    if clip_info is None:
                        ...
                        if len(clip_timestamp) == 16:
                            ...
                            clip_starting_timestamp = (
                                clip_starting_timestamp.astimezone(get_localzone())
                            )
                        else:
                            ...
                            clip_starting_timestamp = (
                                clip_starting_timestamp.astimezone(timezone.utc)
                            )
...
def create_intermediate_movie(
    ...
    _exclude = object()
    for camera in video_settings["video_layout"].clip_order:
        if clip_filename := clip_filenames.get(camera, _exclude):
            # If camera is not in dictionary then it is excluded.
            if clip_filename is not _exclude:
                # Got a valid clip for this camera and to be included
                ffmpeg_camera_commands.append(
                    ffmpeg_offset_command + ["-i", clip_filename]
                )
                ffmpeg_camera_filters.append(
                    ";["
                    + str(len(ffmpeg_camera_commands) - 1)
                    + ":v] "
                    + video_settings["cameras"][camera]
                )
        else:
            ffmpeg_camera_filters.append(
                video_settings["background"].format(
                    ...
                    width=video_settings["video_layout"].cameras(camera).width,
                    height=video_settings["video_layout"].cameras(camera).height,
                )
                + f"[{camera}]"
            )
    ...
    ffmpeg_filter = video_settings["base"].format(
    ...
    for ffmpeg_camera_command in ffmpeg_camera_commands:
        ffmpeg_command += ffmpeg_camera_command
...
def process_folders(source_folders, video_settings, delete_source):
    ...
    for event_count, event_folder in enumerate(sorted(event_list)):
        ...
        # No processing, add to list of movies to merge if what was provided is
        # just a file
        ...
        # If set for Sentry then offset is only used for clips with reason Sentry
        # and having a event timestamp.
    ...
    # output a specific filename was provided not matching the filename for
    # the event clip
    movies_list = None
    if movies:
            ...
            for movie in sorted(movies.keys()):
                ...
                if create_movie(
                    ...
                    if movies.get(movie).filename is not None:
                        ...
                        # Delete the 1 event movie if we created the movie because there
                        # was only 1 folder.
...
def main() -> int:
    ...
    epilog = (
        ...
        if internal_ffmpeg else
        "This program requires ffmpeg which can be downloaded from: https://ffmpeg.org/download.html")
    ...
    monitor_group.add_argument(
        "--monitor_trigger", required=False, type=str,
    ...
    layout_group.add_argument(
        ...
        default="FULLSCREEN", type=str.upper,
    ...
    layout_group.add_argument(
        "--camera_position", dest="clip_pos", type=str.lower, nargs="+",
        action="append",
        help="R|Set camera clip position within video. Selecting this will override the layout selected!\n"
        "The camera clip scale will be set to 2896x1876, use scale to adjust accordingly.\n"
        "Default layout is:\n"
        "    Front: 0x0\n"
        "    Rear: <width front>x0\n"
        ...
        "  --camera_position camera=rear  2896x1876                        Position rear camera at 2896x1876\n",)
    ...
    layout_group.add_argument(
        "--scale", dest="clip_scale", type=str.lower, nargs="+", action="append",
        help="R|Set camera clip scale for all clips, scale of 1 is 2896x1876 camera clip.\n"
        ...
        "  --scale 0.5 --scale camera=front 1                      all are 640x480 except front at 2896x1876\n"
        "  --scale camera=left .25 --scale camera=right 320x240    left and right are set to 320x240\n"
        "Defaults:\n"
        "    WIDESCREEN: 1/2 (front 2896x1876, others 640x480, video is 1920x1920)\n"
        "    FULLSCREEN: 1/2 (640x480, video is 1920x1876)\n"
        "    CROSS: 1/2 (640x480, video is 2896x1440)\n"
        "    DIAMOND: 1/2 (640x480, video is 1920x976)\n",)
    layout_group.add_argument(
        "--mirror", dest="rear_or_mirror", action="store_const", const=1,
    ...
    layout_group.add_argument(
        "--rear", dest="rear_or_mirror", action="store_const", const=0,
    ...
    layout_group.add_argument(
        "--no-swap", dest="swap_leftright", action="store_const", const=0,
    ...
    text_overlay_group.add_argument(
        "--text_overlay_fmt", required=False, type=str,
    ...
    text_overlay_group.add_argument(
        "--timestamp_format", required=False, type=str, default="%x %X",
    ...
    offset_group.add_argument(
        "--sentry_offset", dest="sentry_offset", action="store_true",
    ...
    output_group.add_argument(
        "--chapter_offset", dest="chapter_offset", type=int, default=0,
    ...
    output_group.add_argument(
        "--merge", required=False, dest="merge_group_template", type=str, nargs="?",
    ...
    output_group.add_argument(
        "--merge_timestamp_format", required=False, type=str, default="%Y-%m-%d_%H_%M",
    ...
    advancedencoding_group.add_argument(
        "--no-faststart", dest="faststart", action="store_true",
    ...
    advancedencoding_group.add_argument(
        "--encoding", required=False, choices=["x264", "x265"],
        type=str.lower, default=argparse.SUPPRESS,
    ...
    advancedencoding_group.add_argument(
        "--enc", required=False, type=str, default=argparse.SUPPRESS,
    ...
    mirror.update(
        {
            "front": mirror["left"] if layout_settings.swap_front_rear else "",
            "rear": mirror["left"] if not layout_settings.swap_front_rear else "",
        }
    )
    ...
    for camera in layout_settings.clip_order:
        if layout_settings.cameras(camera).include:
            ffmpeg_camera.update(
                {
                    camera: (
                        "setpts=PTS-STARTPTS, "
            ...
            ffmpeg_video_position += ";[{input_clip}][{camera}] overlay=eof_action=pass:repeatlast=0:x={x_pos}:y={y_pos} [{camera}1]".format(
    ...
    if not args.no_timestamp and text_overlay_format is not None:
        ...
        ffmpeg_timestamp = (
            ffmpeg_timestamp +
            f"drawtext=fontfile={layout_settings.font.font}:"
